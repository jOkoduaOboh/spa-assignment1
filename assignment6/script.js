...
const processTouch = async (e, boardID) => {
    ...
    if (x.charCodeAt(0) >= 65 && x.charCodeAt(0) <= 90) {
        console.log("greater")
        if (input != "") {
            const wordPos = boards[boardID].wordPos
            const letterPos = boards[boardID].letterPos
            console.log("This is: ", input)
            boards[boardID].letter = input.toLowerCase();
